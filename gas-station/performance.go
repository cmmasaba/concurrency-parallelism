...
func startUtil() func() (walltime, exectime time.Duration) {
	...
	return func() (walltime time.Duration, exectime time.Duration) {
		_ = syscall.Getrusage(syscall.RUSAGE_SELF, &ru2)
		return time.Since(start), time.Duration(syscall.TimevalToNsec(ru2.Utime) - syscall.TimevalToNsec(ru1.Utime))
	}
}

// perfTest runs f repeatedly until arg.dur elapses, then retuens a
// perfResult containing up to maxSamples uniformly selected durations.
// if arg.interval > 0, it specifies the rate at which to attempt requests
// and increment res.drops on failures
func perfTest(arg perfArg, f func()) (res perfResult) {
	// Pipeline stages: request generator -> workers -> sampler
	endUtil := startUtil()

	// Generate requests until arg,durs elapses
	...
	go func() {
		...
		for {
			if send == nil {
				...
				// Attempt to send requst periodically, drop if queue is full
				select {
				case <-stop.C:
					return
				case <-send.C:
	...
	// Workers run f until requests channel is closed
	...
	for range arg.par {
		go func() {
			...
			for start := range requests {
				...
				_ = queueTime // Time spent in the queue, not currently used
				start = time.Now()
...
// A perfResult summarizes the result of a performance test including
